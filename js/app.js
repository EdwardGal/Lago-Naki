...
// LAZY LOAD IMG
// Для конструкции с одной img указать data-src="путь-к-картинке", а в src="путь-на-заглушку"
// Для конструкции с picture указать data-src="путь-к-картинке", а в src="путь-на-заглушку"
...
if (lazyImages.length > 0) {
	lazyImages.forEach(img => {
		...
		let srcWebP = `${src.replace(/\.[^.]*$/, '')}.webp`;
		if (picture) {
			let childes = picture.children;
			for (let index = 0; index < childes.length; index++) {
				const child = childes[index];
				if (!child.dataset.srcset && !child.dataset.src) {
					child.setAttribute('data-srcset', `${srcWebP}`);
				}
			}
		}
		lazyLoad(img);
...
let lazySource = document.querySelectorAll('source[data-srcset]');
if (lazySource.length > 0) {
	lazySource.forEach(img => {
		lazyLoad(img);
	});
}
...
if (document.querySelector('.intro__slider')) {
	new Swiper('.intro__slider', {
		...
		// Arrows
		navigation: {
			nextEl: '.slider-intro .slider-arrow_next',
			prevEl: '.slider-intro .slider-arrow_prev',
		}
	});
}


if (document.querySelector('.waterfall__slider')) {
	new Swiper('.waterfall__slider', {
		observer: true,
		observeParents: true,
		slidesPerView: 'auto',
		spaceBetween: 20,
		...
		// Arrows
		navigation: {
			nextEl: '.slider-waterfall .slider-arrow_next',
			prevEl: '.slider-waterfall .slider-arrow_prev',
		}
	});
}

if (document.querySelector('.skiing__slider')) {
	new Swiper('.skiing__slider', {
		observer: true,
		observeParents: true,
		slidesPerView: 'auto',
		spaceBetween: 20,
		...
		// Arrows
		navigation: {
			nextEl: '.slider-skiing .slider-arrow_next',
			prevEl: '.slider-skiing .slider-arrow_prev',
		}
	});
}

if (document.querySelector('.springs__slider')) {
	new Swiper('.springs__slider', {
		observer: true,
		observeParents: true,
		slidesPerView: 'auto',
		spaceBetween: 20,
		...
		// Arrows
		navigation: {
			nextEl: '.slider-springs .slider-arrow_next',
			prevEl: '.slider-springs .slider-arrow_prev',
		}
	});
}

if (document.querySelector('.caves__slider')) {
	new Swiper('.caves__slider', {
		observer: true,
		observeParents: true,
		slidesPerView: 'auto',
		spaceBetween: 20,
		...
		watchSlidesVisibility: true,
		pagination: {
			el: '.slider-caves__dotts',
			clickable: true,
		},
		// Arrows
		navigation: {
			nextEl: '.slider-caves .slider-arrow_next',
			prevEl: '.slider-caves .slider-arrow_prev',
		}
	});
}
...
const start = datepicker('.start', {
	...
	overlayPlaceholder: 'Год (4 цифры)',
	startDay: 1,
	maxDate: new Date(2023, 0, 1), // Jan 1st, 2099.
	minDate: new Date(2021, 5, 25), // June 1st, 2016.
	id: 1
})

const picker = datepicker('.end', {
	formatter: (input, date, instance) => {
		const value = date.toLocaleDateString()
		input.value = value
	},
...
if (link) {
	...
	for (let index = 0; index < link.length; index++) {
		...
		el.addEventListener('click', function (e) {
			...
			_goto(target_block, 600, headerHeight);
			e.preventDefault();
		})
	}
...
if (paralaxMouse.length > 0) {
	paralaxMouse.forEach(el => {
		...
		let paramСoefficientX = 500;
		let paramСoefficientY = 1000;
		let paramDirectionX = "true";
		let paramDirectionY = "true";
		let paramAnimation = 20;
